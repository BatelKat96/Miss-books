...
function query(txt) {
  ...
  const url = `https://www.googleapis.com/books/v1/volumes?q=${searchWord}+terms`

  const googleSearchedBooks = utilService.loadFromStorage(BOOK_SEARCH_KEY) || {}
  if (googleSearchedBooks[searchWord]) {
    const { booksOptions } = googleSearchedBooks[searchWord]
    console.log('booksOptions:', booksOptions)

    console.log('FROM CACHE')

    return Promise.resolve(booksOptions)
  }

  return axios.get(url).then(res => {
    const booksOptions = res.data.items
    googleSearchedBooks[searchWord] = {
      booksOptions,
      lastReq: Date.now()
    }
    utilService.saveToStorage(BOOK_SEARCH_KEY, googleSearchedBooks)
...
function debounce(func, wait) {
  ...
  return function executedFunction(...args) {
    return new Promise((resolve) => {// optional for working with apis

      const later = () => {
        clearTimeout(timeout)
        const res = func(...args)
        return resolve(res) // optional for working with apis
      };
